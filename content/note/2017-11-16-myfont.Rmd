...
# Another digression: Font embedding 

이건 문제가 될 수도 있고 아닐 수도 있다. 하지만 문제가 될 것 같으면 확실히 해두면 되겠다. 
...
q <- ggplot(mtcars, aes(x=wt, y=mpg)) + geom_point() +
  ...
  theme(title=element_text(family="notosanskr", size = 12),
        axis.title=element_text(family="notosanskr", size = 8),
        strip.text=element_text(family="notosanskr", size = 8))

print(q);
```

```{r eval=FALSE}
ggsave(p, filename="./foo.pdf", device=cairo_pdf)
```

위 처럼 명시적으로 device를 지정해주면 `foo.pdf` 문서에 font가 확실하게 문서에 임베딩된다. 
