...
R에는 Non Standard Evaluation(NSE)라고 불리는 녀석이 있다. 어떤 선배님은 변수를 선언하지 않는 R이 처음부터 무척 마음에 들지 않았다, 라고 툴툴대셨지만, 대충 알아서 해주는 건 사실 단점보다는 장점이 많다. 좌간, NSE 역시 이와 비슷한 차원에서 이해하면 될 것 같다. 예를 보자. 
...
둘 다 먹히지 않는다. 나는 df에서 x라는 컬럼을 택하고 싶다. 다만 NSE를 이용해서 하는 게 아니라 다른 방법으로 하고 싶다. 더 구체적으로 말해보자. 내가 택하는 변수를 '임의화'하여 이를 함수를 만드는 데 활용하고 싶다. NSE에 익숙해지면 위에서 보다시피 이런 단순한 작업이 생각만큼 쉽지 않다. 물론 이런 NSE의 문제점을 뛰어넘기 위해서는 quosures와 tidyval의 개념을 두루두루 역혀두는 편이 좋다.[^tidyval] 하지만 역시 야매정신과 지금만 대충 때우는 정신으로 무장한 나로서는 쉽게 떠올리기 좋은 간편한 해결책에 눈길이 더 간다. 
...
때로는 지적 결벽보다는 필요한 것 만큼만 익히는 야매도 나쁘지 않다. 
